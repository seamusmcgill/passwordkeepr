...
module.exports = (db) => {
  ...
  const getUserWithId = function(id) {
    ...
    return db.query(`
    SELECT users.full_name, users.email, users.id, organizations.name AS organization_name, organizations.logo_url AS logo_url
    FROM users JOIN organizations ON users.organization_id = organizations.id
    WHERE users.id = $1`, [id])
      .then(result => {
        if (result.rows.length === 0) {
  ...
  router.get("/current", (req, res) => {
    ...
    getUserWithId(userId)
      .then(user => {
        ...
        res.send({user: {name: user.full_name, email: user.email, organization: user.organization_name, logo_url: user.logo_url, id: userId}});
      })
      .catch(e => res.send(e));
  });
